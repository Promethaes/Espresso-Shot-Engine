...
Espresso::TestScene::TestScene(bool firstScene)
	:Scene(firstScene), specularMap("", TextureType::DiffuseMap)
{
	...
	specularMap = Texture(std::string(std::getenv("EspressoShotPath")) + "Assets/Textures/Metal_specmap.png",TextureType::SpecularMap);
	for (int i = 0; i < 10; i++) {
		cubes.push_back(Cube(vertices2, 288, new Texture(std::string(std::getenv("EspressoShotPath")) + "Assets/Textures/container2.png",TextureType::DiffuseMap), true));
...
bool Espresso::TestScene::init()
{
	
	return initialized = true;
}

bool Espresso::TestScene::exit()
{
	return shouldExit = true;
}

void Espresso::TestScene::childUpdate(float dt,Camera& defaultCamera)
{
	//centre cube
